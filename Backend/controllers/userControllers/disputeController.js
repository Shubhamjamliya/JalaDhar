...
const createDispute = async (req, res) => {
  try {
    ...
    // Send notification to all admins and vendor if booking exists
    try {
      ...
      for (const admin of admins) {
        await sendNotification({
          ...
          message: `User raised a new dispute: ${finalSubject}`,
          relatedEntity: {
            entityType: 'Dispute',
      ...
      if (booking && booking.vendor) {
        await sendNotification({
          ...
          message: `Customer raised a dispute on booking #${booking._id.toString().slice(-6)}: ${finalSubject}`,
          relatedEntity: {
            entityType: 'Dispute',
            entityId: dispute._id
          },
