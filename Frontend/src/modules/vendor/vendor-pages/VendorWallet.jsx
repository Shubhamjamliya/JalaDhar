...
export default function VendorWallet() {
    ...
    return (
        ...
        <PageContainer>
            ...
            <section className="relative my-3 overflow-hidden rounded-3xl bg-gradient-to-br from-[#0F52BA] via-[#0A84FF] to-[#00C49F] p-6 text-white shadow-xl shadow-cyan-500/15 border border-cyan-400/30">
                ...
                <div className="relative z-10 flex flex-col items-center text-center">
                    <div className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full bg-white/20 backdrop-blur-md text-xs font-bold text-cyan-100 border border-white/25 mb-2 shadow-2xs">
                        <span>💧 Jaladhaara Hydro-Payouts</span>
                    </div>
            ...
            {/* Hydro Summary Metric Cards */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                <div className="rounded-2xl bg-white p-3.5 shadow-xs border border-gray-100/90 hover:border-emerald-300 transition-all">
                    <span className="material-symbols-outlined text-emerald-500 !text-2xl font-bold">payments</span>
                    <p className="mt-1.5 text-xs font-bold text-gray-500">Total Credited</p>
                    <p className="mt-0.5 text-base font-extrabold text-emerald-600">
                        ₹{formatAmount(totalCredited)}
                    </p>
                </div>
                <div className="rounded-2xl bg-white p-3.5 shadow-xs border border-gray-100/90 hover:border-rose-300 transition-all">
                    <span className="material-symbols-outlined text-rose-500 !text-2xl font-bold">remove_circle</span>
                    <p className="mt-1.5 text-xs font-bold text-gray-500">Total Deducted</p>
                    <p className="mt-0.5 text-base font-extrabold text-rose-600">
                        ₹{formatAmount(totalDeducted)}
                    </p>
                </div>
                <div className="rounded-2xl bg-white p-3.5 shadow-xs border border-gray-100/90 hover:border-blue-300 transition-all">
                    <span className="material-symbols-outlined text-[#0A84FF] !text-2xl font-bold">calendar_month</span>
                    <p className="mt-1.5 text-xs font-bold text-gray-500">This Month</p>
                    <p className="mt-0.5 text-base font-extrabold text-[#0A84FF]">
                        ₹{formatAmount(thisMonthEarnings)}
                    </p>
                </div>
                <div className="rounded-2xl bg-white p-3.5 shadow-xs border border-gray-100/90 hover:border-teal-300 transition-all">
                    ...
                    <p className="mt-0.5 text-base font-extrabold text-teal-700">
                        ₹{formatAmount(walletBalance)}
                    </p>
                </div>
            </div>
                ...
                {transactions.length === 0 ? (
                    ...
                    transactions.map((transaction) => {
                        ...
                        return (
                            ...
                            >
                                <div className={`flex h-12 w-12 items-center justify-center rounded-2xl shrink-0 ${
                                    isSuccess ? (isCredit ? "bg-emerald-50 text-emerald-600 border border-emerald-100" : isDebit ? "bg-amber-50 text-amber-600 border border-amber-100" : isWithdrawal ? "bg-blue-50 text-[#0A84FF] border border-blue-100" : "bg-gray-50 text-gray-500") : 
