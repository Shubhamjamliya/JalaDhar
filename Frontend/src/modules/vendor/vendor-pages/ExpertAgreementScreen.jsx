...
export default function ExpertAgreementScreen() {
    ...
    const [verificationStatus, setVerificationStatus] = useState('VERIFIED_PENDING_AGREEMENT');
    ...
    const fetchAgreementStatus = async () => {
        try {
            ...
            if (response.data?.success && response.data?.data) {
                const { activeVersion, agreementText, verificationStatus, expertId, requiresAcceptance } = response.data.data;
                setAgreementVersion(activeVersion || 'v1.0');
                setAgreementText(agreementText || '');
                setVerificationStatus(verificationStatus || 'PENDING');
                setExpertId(expertId || '');

                if (verificationStatus === 'ACTIVATED' && !requiresAcceptance) {
                    setIsActivated(true);
                    setIssuedExpertId(expertId || 'EX-2026-ACTIVE');
                }
            }
        } catch (err) {
            console.error('Error fetching Expert Agreement status:', err);
            toast.showError('Failed to load Expert Onboarding Agreement');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleAcceptAndActivate = async () => {
        ...
        try {
            ...
            if (response.data?.success) {
                const assignedId = response.data.data?.expertId || expertId || 'EX-2026-ACTIVE';
                setIsActivated(true);
                setIssuedExpertId(assignedId);
                toast.showSuccess('Congratulations! Expert Onboarding Agreement accepted & Verified Expert ID issued!');
            } else {
                toast.showError(response.data?.message || 'Failed to activate account');
            }
        } catch (err) {
            console.error('Error accepting Expert Agreement:', err);
    ...
    if (isActivated) {
        return (
            <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4 sm:p-6 animate-fadeIn">
    ...
    return (
        <div className="min-h-screen bg-slate-100 py-6 px-4 sm:px-6 flex items-center justify-center">
            <div className="w-full max-w-3xl bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
                ...
                {/* Banner Header */}
                <div className="bg-gradient-to-r from-blue-700 via-indigo-700 to-blue-800 p-5 sm:p-6 text-white shrink-0">
                ...
                <div className="p-4 bg-emerald-50 border-b border-emerald-100 flex items-start gap-3 shrink-0">
                    ...
                    <div className="text-xs text-emerald-950">
                        <strong className="font-extrabold block">Admin Document Verification Complete:</strong>
                        Your Aadhaar, PAN, Bank Details, and Qualifications have been approved. Accept this click-wrap agreement to generate your <strong>Verified Expert ID</strong> and activate booking acceptance.
                    </div>
                </div>
                ...
                <div className="p-4 sm:p-5 bg-slate-50 border-t border-slate-200 shrink-0 space-y-3">
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-3">
                        <button
                            type="button"
                            onClick={() => navigate('/vendor/dashboard')}
                            className="w-full sm:w-auto px-5 py-3 rounded-2xl border border-slate-200 font-bold text-xs text-slate-600 hover:bg-slate-100"
                        >
                            Decline & Exit
                        </button>
                        ...
                        <button
                            ...
                            className={`w-full flex-1 py-3.5 px-6 rounded-2xl font-black text-sm transition-all flex items-center justify-center gap-2 shadow-md ${
            ...
            <ExpertAgreementDocViewer
                ...
                expertId={issuedExpertId || expertId}
            />
        </div>
    );
}
