...
export default function UserCreateDispute() {
    ...
    return (
        <div className="max-w-2xl mx-auto space-y-6 pb-12">
            {/* Header Banner */}
            <div className="bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 rounded-3xl p-6 text-white shadow-xl shadow-blue-950/20 relative overflow-hidden">
                <div className="absolute -right-8 -bottom-8 w-40 h-40 bg-blue-500/10 rounded-full blur-2xl pointer-events-none" />
                <div className="relative z-10 flex items-start justify-between">
                    <div>
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-500/20 border border-blue-400/30 text-blue-200 text-xs font-semibold uppercase tracking-wider mb-3">
                            <IoHelpCircleOutline className="text-sm" /> Resolution Center
                        </span>
                        <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight">Raise a Dispute</h1>
                        <p className="text-sm text-blue-200/80 mt-1 font-medium max-w-md">
                            Describe your issue in detail. Our support team will review your case and respond promptly.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 sm:p-8 space-y-6">
                ...
                <div className="space-y-2">
                    <CustomDropdown
                        ...
                        activeColor="blue"
                        placeholder="Select a booking reference (optional)"
                        options={[
                            { value: "", label: "Select a booking reference (optional)" },
                            ...(loadingBookings
                                ? [{ value: "", label: "Loading your bookings..." }]
                                : bookings.map(booking => ({
                                    value: booking._id,
                                    label: `#${booking._id.toString().slice(-8).toUpperCase()} — ${booking.service?.name || "Survey Service"} (${new Date(booking.scheduledDate).toLocaleDateString("en-IN", { day: 'numeric', month: 'short' })})`
                                }))
                            )
                        ]}
                    />
                ...
                <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-2">
                        <IoAlertCircleOutline className="text-blue-500 text-base" />
                ...
                <div className="space-y-2">
                    ...
                    <textarea
                        ...
                        onChange={handleInputChange}
                        rows={5}
                        placeholder="Please describe what went wrong in detail so we can investigate and resolve it for you quickly..."
                        className={`w-full p-4 bg-slate-50/70 border rounded-2xl text-sm font-medium text-slate-800 placeholder:text-slate-400 focus:bg-white focus:ring-4 outline-none transition-all resize-y ${
                            errors.description
                                ...
                                : "border-slate-200 focus:border-[#0A84FF] focus:ring-blue-50"
                        }`}
                    />
                ...
                <div className="space-y-3">
                    <label className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center justify-between">
                        <span className="flex items-center gap-2">
                            <IoCloudUploadOutline className="text-blue-500 text-base" />
                    ...
                    <label
                        htmlFor="attachments"
                        className="group flex flex-col items-center justify-center p-6 border-2 border-dashed border-slate-200 hover:border-[#0A84FF] rounded-2xl bg-slate-50/50 hover:bg-blue-50/30 transition-all cursor-pointer text-center"
                    >
                        <div className="w-12 h-12 rounded-2xl bg-blue-100/80 group-hover:bg-[#0A84FF] group-hover:text-white text-[#0A84FF] flex items-center justify-center mb-3 transition-colors shadow-sm">
                            <IoCloudUploadOutline className="text-2xl" />
                        </div>
                        <p className="text-sm font-bold text-slate-800 group-hover:text-[#0A84FF] transition-colors">
                            ...
                            {attachments.map((file, index) => (
                                ...
                                >
                                    <div className="flex items-center gap-3 min-w-0 flex-1">
                                        <div className="p-2 bg-blue-100 text-[#0A84FF] rounded-xl shrink-0">
                ...
                <div className="flex items-center gap-3 pt-4 border-t border-slate-100">
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="flex-[2] py-3.5 px-6 rounded-2xl bg-gradient-to-r from-[#0A84FF] to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white text-sm font-bold shadow-lg shadow-blue-500/25 transition-all flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer"
                    >
                        {loading ? (
                            <>
